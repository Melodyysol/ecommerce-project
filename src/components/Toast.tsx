  ...
  return (
    ...
    >
      <div className="relative items-center flex gap-4 py-4 pl-3 rounded border border-gray-100">
        <div
          className={`${type === "error" ? "bg-red-500 " : "bg-green-500 "} h-5 w-5 rounded-full text-white text-center`}
        >
          !
        </div>
        <p>{message}</p>
        ...
        <motion.div
          initial={{ width: "100%" }}
          animate={controls}
          className={`h-1 ${type === "error" ? "bg-red-500 " : "bg-green-500 "} absolute bottom-0 left-0 rounded-bl`}
        ></motion.div>
      </div>
    </motion.div>
  );
